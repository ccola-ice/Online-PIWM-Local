...
class ControlledVehicle:
    def __init__(self, start_end_state, delta_time, discrete_action_num, max_speed=9., consider_jerk_threshold=False):
        ...
        # self._consider_jerk_threshold = consider_jerk_threshold
        # if self._consider_jerk_threshold:
        #     self._jerk_threshold = 20
        #     self._prev_acc = 0.
    ...
    def update_state(self, acc_norm, steering_norm):
        ...
        # clip acc considering comfort/jerk threshold
        # acceleration regulation: changes cannot happen abruptly, consider a filter that regulize the acc(m/s^2) considering jerk(m/s^3)
        # if self._consider_jerk_threshold:
        #     jerk = abs(acc - self._prev_acc) / 0.1
        #     if jerk > self._jerk_threshold:
        #         acc = np.clip(acc, self._prev_acc - self._jerk_threshold * 0.1, self._prev_acc + self._jerk_threshold * 0.1)
        #     print('jerk is:', abs(acc - self._prev_acc) / 0.1)
        #     self._prev_acc = acc
