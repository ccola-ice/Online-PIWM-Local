...
class InteractionEnv:
    def __init__(self, settings):
        # TODO: dmv3 settings type in str 
        ...
        if self._loader_type == 'prediction':
            prediction_tracks_dir = os.path.join(root_dir, 'prediction', self._map_name)
            self._track_loader = PredictionLoader(prediction_tracks_dir, groundtruth_tracks_dir, ego_num=1, only_trouble=settings['only_trouble'], eval=self._eval)
    ...
    def change_track(self):
        ...
        if self._loader_type == 'prediction':
            gt_csv_index = self._track_loader.change_track_file(file_name='5_45800_55800_33P0.pkl') # for small scale experiment
        elif self._loader_type == 'dataset': 
    ...
    def step(self, action_dict, prediction=None):
        ...
        for ego_id in result_dict.keys():
            ...
            # TODO: set a reward manager, and using ego_state info instead of using observation info
            # terminal reward
        ...
        if self._visualization:
            self._render.update_param(self._map)
            # specified ego(with/without ghost ego) and selected vehicle highlight
            # TODO: is function 'get_surrounding_vehicle_id' necessary?
            surrounding_vehicle_id_list = self._observation.get_surrounding_vehicle_id(self.observation_dict)
            self._render.render_vehicles(surrounding_vehicle_id_list, self._ghost_visualization)
            ...
            # TODO: render() takes too much time
            self._render.render()
            # save images
            if self.save_images:
                self._render.save_images(ego_id, self._current_time)
                ...
                if term_condition == 'collision':
                    for ego_id in ego_id_list:
                        if observation_dict[term_condition][ego_id] is True:
                            # TODO: set a option concerning whether collision is a terminal condition
                            if self._eval:
                                done_dict[ego_id] = True
                                # done_dict[ego_id] = False
                            else:
    ...
    def get_ground_truth_route(self, interval_distance=2.):
        ...
        for vehicle_type in ['ego', 'npc']:
            ...
            # # if proper start and end lanelet exist
            # route_lanelet = self.get_route_lanelet(start_lanelet, end_lanelet)
            # route_lanelet_dict[vehicle_id] = route_lanelet
